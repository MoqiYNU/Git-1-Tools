...
public class DotUtils {
	...
	public void pt2Dot(ProTree pt, Map<String, String> map, String name) throws Exception {
		...
		for (Node node : nodes) {
			...
			List<String> chaNodeIdfs = node.getChaNodes();
			for (String chaNodeIdf : chaNodeIdfs) {
				Node chaNode = proTreeUtils.getNodeByIdf(pt, chaNodeIdf);
				if (chaNode.getType().equals("leaf")) {
					String edge = node.getIdf() + "->" + "\"" + map.get(chaNodeIdf) + "\""  + ";";
					buffer.append(edge + "\n");
				}else {
					String edge = node.getIdf() + "->" + chaNodeIdf + ";";
					buffer.append(edge + "\n");
				}
		   }
		   buffer.append("\n");
