...
public class PlanUtils {
	...
	public LTS executeETS(List<ProNet> proNets, List<Bag> initBags) {
		
		System.out.println("begin execute................................" + "\n");
		
		int ECCIndex = deterECC(proNets);//确定ECC位置
		...
		while(visitingQueue.size() > 0){
			...
			for (String tran : enableTrans) {
				...
				// 3.2定义全局可用计划集
				List<Integer> globalAvailPlans = null;
				
				if (proNetIndex == ECCIndex) {//Case1:ECC中变迁执行,不影响其他附庸应急组织
					
					//Note:全局可用计划为ECC中可用计划集
					globalAvailPlans = bagsFrom.get(proNetIndex).getAvailPlans();
					//若无可用计划,则跳过
				...
				}else {//Case2:非ECC中变迁执行,影响ECC
					
					//Note:全局可用计划为当前附庸应急组织中可用计划与ECC中可用计划的交集
					globalAvailPlans = (List<Integer>) CollectionUtils.intersection(
							bagsFrom.get(proNetIndex).getAvailPlans(), 
							bagsFrom.get(ECCIndex).getAvailPlans());
					//若无可用计划,则跳过
					if (globalAvailPlans.size() == 0) {
						continue;
					}
                    ...
                    //定义到达组合状态(Note:需同时更新ECC和当前附庸应急组织的包)
                    List<String> placesTo = PetriUtils.getPlacesTo(compNet, placesFrom, tran);
    				Marking markingTo = new Marking();
    				markingTo.setPlaces(placesTo);
                    List<Bag> bagsTo = new ArrayList<Bag>();
                    for (int i = 0; i < bagsFrom.size(); i++) {
                    	if (i == ECCIndex) {//C1:更新ECC包中可用计划集
                    		Bag bag = new Bag();
    						bag.setExecActs(bagsFrom.get(i).getExecActs());
    						bag.setAvailPlans(localAvailPlans);
    						bag.setMaps(bagsFrom.get(i).getMaps());
    						bagsTo.add(bag);
						}else if (i == proNetIndex) {//C2:更新当前附庸应急组织包中已执行活动集和可用计划集
							Bag bag = new Bag();
    						List<String> execActsTo = new ArrayList<String>();
    						execActsTo.addAll(bagsFrom.get(i).getExecActs());
    						execActsTo.add(tran);
	...
	//确定一个应急处置系统中ECC位置
	public int deterECC(List<ProNet> proNets) {
		
		int ECCIndex = 0;
		for (int i = 1; i < proNets.size(); i++) {
			if (proNets.get(i).getMsgPlaces().size()
					> proNets.get(ECCIndex).getMsgPlaces().size()) {
				ECCIndex = i;
			}
		}
		return ECCIndex;
	...
	/****************************实现面向执行计划执行(Note:协同业务过程)*****************************/
	
	public LTS executeCBP(List<ProNet> proNets, List<Bag> initBags) {
	...
	public List<Bag> enforce(List<ProNet> proNets) throws Exception {
		
		// 1.生成每个语义兼容组合片段,即计划
		List<CompFrag> plans = getSemtCompaCompFrags(proNets);
		
		// 2.打印组合片段..............
		PrintUtils.printCompFrag(plans, proNets);
		
		// 3.生成每个业务过程配置初始包(Note:可用计划为全部计划,已执行活动为空集)
		int planNum = plans.size();//确定全部计划
		List<Bag> initBags = new ArrayList<Bag>();
		for (int i = 0; i < proNets.size(); i++) {
			Bag bag = new Bag();
	...
	public ORDMatrix getORDMatrixFromFrag(ProTree frag) {
		...
		//根节点是最后一个
		Node root = nodes.get(nodes.size()-1);
		List<String> acts = proTreeUtils.getLeafDesc(frag, root);
	...
	public List<CompFrag> getSemtCompaCompFrags(List<ProNet> proNets) throws Exception {
		...
		for (CompFrag compFrag : syncCompaFrags) {
			...
			if (isDAG(interGraph)) {//1.语义兼容
				//semtCompaFrags.add(compFrag);
				if (!isResConfict(proNets, compFrag)) {//2.无资源冲突
	...
	//判断组合片段中是否存在资源冲突
	private boolean isResConfict(List<ProNet> proNets, CompFrag compFrag) {
		...
		// Step 2.获取组合过程中资源信息
		List<String> resPlaces = compNet.getResPlaces();
		List<String> resources = compNet.getResources();
		Map<String, Integer> resProperMap = compNet.getResProperMap();
		
		// Step 3.计算可用和请求资源向量
		/**C1:获取组合网中可用资源...........................................*/
		int[] avilRes = new int[resPlaces.size()];
		org.apache.commons.collections4.Bag<String> bagAvail = new HashBag<String>(resources);
		Set<String> uniqueSetAvail = bagAvail.uniqueSet();
		for (String res : uniqueSetAvail) {
			int index = resPlaces.indexOf(res);
			int number = bagAvail.getCount(res);
			avilRes[index] = number;
		}
		/**C2:获取组合片段中请求资源...........................................*/
        int[] reqRes = new int[resPlaces.size()];
        for(int i = 0; i < resPlaces.size(); i++){//初始为0
			reqRes[i] = 0;
		}
        List<String> actsInCompFrag = getActsInCompFrag(compFrag);
        for (String act : actsInCompFrag) {//计算组合片段中每个活动请求资源
			List<String> reqResPL = (List<String>) CollectionUtils.
					intersection(PetriUtils.getPreSet(act, compNet.getFlows()), resPlaces);
			org.apache.commons.collections4.Bag<String> bagReq = new HashBag<String>(reqResPL);
			Set<String> uniqueSetReq = bagReq.uniqueSet();
			for (String res : uniqueSetReq) {
				int index = resPlaces.indexOf(res);
				int number = bagReq.getCount(res);
				reqRes[index] = reqRes[index] + number;
			}
		}
        System.out.println("avilRes: " + Arrays.toString(avilRes) + ", reqRes" + Arrays.toString(reqRes));
        // Step 4.判断是否存在资源冲突
        for(int i = 0; i < resPlaces.size(); i++){
        	if (avilRes[i] >= reqRes[i]) {
				continue;
			}else {
				if (avilRes[i] >= 1 && resProperMap.get(resPlaces.get(i)) == 0) {
					continue;
				}else {
					return true;
				}
	...
	//获取组合判断中所有变迁集
	public List<String> getActsInCompFrag(CompFrag compFrag){
		List<String> actsInCompFrag = new ArrayList<String>();
		List<ProTree> frags = compFrag.getFrags();
		for (ProTree frag : frags) {
			actsInCompFrag.addAll(getActsInFrag(frag));
		}
		return actsInCompFrag;
			...
			for (Node node : SEQNodes) {
				...
				for (int i = 1; i < chaNodes.size(); i++) {
					...
					List<String> curActs = (List<String>) CollectionUtils.
							intersection(proTreeUtils.
									getLeafDesc(frag, proTreeUtils.getNodeByIdf(frag, chaNodes.get(i))), acts);
					//(0,i-1)节点集中含有的叶子节点集(受限活动集acts)
					for (String curAct : curActs) {
						map.put(curAct, getPreOrdActs(frag, chaNodes, i, acts));
					}
				}
			}
		}
	...
	public List<String> getPreOrdActs(ProTree frag, List<String> chaNodes, 
			int index, List<String> acts) {
		
		List<String> preActs = new ArrayList<String>();
		for (int i = 0; i < index; i++) {
			List<String> tempActs = proTreeUtils.
					getLeafDesc(frag, proTreeUtils.getNodeByIdf(frag, chaNodes.get(i)));
