...
public class GenProTree {
	...
	public ProTree compute(InnerNet net) throws Exception {
	    ...
	    while(visitingQueue.size() > 0){
			...
			if (metaANDBlocksC1.size() > 0) {//至少有1个元并行部件
                ...
                for (String postAct : postActs) {//针对并发块中的每个迁移进行计算
					...
					if (node == null) {//未生成过
						...
						chaNode.setType("leaf");
						ANDNode.addChaNode(chaNode.getIdf());
						tree.addNode(chaNode);
					}else {
						ANDNode.addChaNode(node.getIdf());
					}
				}
			...
			getAndBlock.compute(netFrom.genInverNet());
			...
			if (metaXORBlocks.size() > 0) {//至少有1个元选择块
				...
				for (String tranInXor : transInXor) {
					...
					if (node == null) {//未生成过
						...
						chaNode.setType("leaf");
						XORNode.addChaNode(chaNode.getIdf());
						tree.addNode(chaNode);
					}else {
						XORNode.addChaNode(node.getIdf());
					}
				}
				//添加选择节点
				...
				if (replacedLoopBlocks.size() > 0) {//从entry出发的每个环均是元环
					...
					if (replacedLoopBlocks.size() == 1) {//4.1 从入口出发只有一个元环
						...
						if (node == null) {//未生成过
							...
							chaNode.setIdf(act);
							chaNode.setType("leaf");
							loopNode.addChaNode(chaNode.getIdf());
							tree.addNode(chaNode);
						}else {//生成过,则直接添加至switNode
							loopNode.addChaNode(node.getIdf());
						}
					...
					tree.addNode(xorNode);
					loopNode.addChaNode(xorNode.getIdf());
					tree.addNode(loopNode);
				    }
